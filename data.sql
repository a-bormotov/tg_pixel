...
-- IMPORTANT (unchanged):
--  - ad series counted inside "packs" after Claim/Unlock with gold
...
win AS (
  SELECT
    TIMESTAMPTZ '2026-07-22 15:00:00+00' AS win_start,
    TIMESTAMPTZ '2026-07-29 15:00:00+00' AS win_end
),
...
/* 3) Ads (gold amount from payload) */
ads_only AS (
  SELECT
    e."userId",
    e."createdAt",
...
/* 11) Total gold (claim/unlock + ads credits) */
points AS (
...
SELECT
  ...
  (COALESCE(p."gold", 0)::numeric * (1 + COALESCE(g.rare,0)*0.01 + COALESCE(g.epic,0)*0.03 + COALESCE(g.legendary,0)*0.10)) AS score,
